...
- **Private gallery UX + Manage accordion** — Compact uses cinematic deck cards (no “Private Collection” / cinematic blurb). Cinematic album strip is a snap deck (active larger, neighbors dimmed). Continue hidden when album empty or no resumable session. Manage albums: accordion sections, Save changes when dirty, profile-link icons auto-detect from pasted URL (no platform dropdown). Drive folder picker ignores Android back / backdrop dismiss — Close only. `app/gallery/page.jsx`, `app/gallery/[slug]/page.jsx`, `GalleryAlbumsPanel.jsx`, `GalleryDriveFolderPicker.jsx`, `lib/gallery-profile-links.js`.
